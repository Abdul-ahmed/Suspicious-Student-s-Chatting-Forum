<?php
	...
	if (!isset($_POST['post'])) {
		...
		$question = mysqli_real_escape_string($myConn, $_POST['question']);
		...
		$replace = "***";
		$save = str_ireplace($search_words, $replace, $question);


		if (empty($question)) {
			header("Location: ../feeds.php?question_field_empty");
		} else {

			$sql = "INSERT INTO allposts (title, author, date, time) value ('$save', '$author', '$date', '$time');";
			$query = mysqli_query($myConn, $sql);
			header("Location: ../feeds.php?post_successful");
		}
	}